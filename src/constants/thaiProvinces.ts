...
export function getAllThaiProvinces(): string[] {
  return THAI_PROVINCES.map((p) => p.th);
}

export function getProvinceByThai(thaiName: string): Province | undefined {
  return THAI_PROVINCES.find((p) => p.th === thaiName);
}
